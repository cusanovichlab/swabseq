...
counts = read.table("Saliva_dilutions_96_vs_384.data.txt")
counts = counts[,-2]
metadata = read.table("Saliva_dilutions_96_vs_384.metadata.txt",header=T,sep="\t")
...
pdf("HighConcentration.pdf",height=8,width=8)
test.wide %>%
  mutate(ATCC_Virus_copies = if_else(ATCC_Virus_copies == 0, 0.1, as.numeric(ATCC_Virus_copies))) %>%
  ggplot(aes(x=ATCC_Virus_copies, y=S2_Spike_Ratio, color=Plate_config)) +
  geom_quasirandom(alpha=1, aes(color=Plate_config),size=3) +
  geom_smooth(data=subset(test.wide,as.numeric(ATCC_Virus_copies) >= 0.5),method=lm,se=FALSE) +
  geom_vline(xintercept = 0.2236068) +
...
write.table(test.wide,"Saliva_dilutions_96_vs_384_wide_mat.txt",row.names=F,quote=F,sep="\t")
