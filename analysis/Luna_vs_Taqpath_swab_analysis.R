...
counts = read.table("Luna_vs_taqpath.data.txt")
counts = counts[,-2]
metadata = read.table("Luna_vs_taqpath.metadata.txt",header=T,sep="\t")
...
test.wide = test.wide[,c(1,5:8,16,9,12,3,17:19)]
colnames(test.wide)[1] = "Index"
colnames(test.wide)[2] = "Index1"
colnames(test.wide)[3] = "Index2"
colnames(test.wide)[9] = "S2_Count"

pdf("LunavsTaqPath.pdf",height=8,width=8)
test.wide %>%
  mutate(Twist_RNA_copies = if_else(Twist_RNA_copies == 0, 3.162278, as.numeric(Twist_RNA_copies))) %>%
  ggplot(aes(x=Twist_RNA_copies, y=S2_Spike_Ratio, color = Enzyme)) +
  geom_quasirandom(alpha=1, aes(color=Enzyme),size=3) +
  geom_smooth(data=subset(test.wide,as.numeric(Twist_RNA_copies) > 1),method=lm,se=FALSE) +
  scale_color_manual(values=c("#EBCC2A","#3B9AB2")) +
  geom_vline(xintercept = 5.623413) +
  scale_x_log10(breaks = c(10^(0:4)), labels = c(0,10^(1:4))) +
...
test.wide %>%
  mutate(Twist_RNA_copies = if_else(Twist_RNA_copies == 0, 3.162278, as.numeric(Twist_RNA_copies))) %>%
  ggplot(aes(x=Twist_RNA_copies, y=S2_Spike_Ratio)) +
...
write.table(test.wide,"Luna_vs_taqpath_wide_mat.txt",row.names=F,quote=F,sep="\t")
