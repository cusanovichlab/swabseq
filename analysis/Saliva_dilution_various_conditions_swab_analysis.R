...
counts = read.table("Saliva_dilution_various_conditions.data.txt")
counts = counts[,-2]
metadata = read.table("Saliva_dilution_various_conditions.metadata.txt",header=T,sep="\t")
...
test.wide = test.wide[,c(1,5:8,17,9,11:13,3,18:20)]
colnames(test.wide)[1] = "Index"
colnames(test.wide)[2] = "Index1"
colnames(test.wide)[3] = "Index2"
colnames(test.wide)[11] = "S2_Count"

pdf("various_conditions.pdf",height=8,width=8)
test.wide %>%
  mutate(ATCC_Virus_copies = if_else(ATCC_Virus_copies == 0, 3.162278, as.numeric(ATCC_Virus_copies))) %>%
  ggplot(aes(x=ATCC_Virus_copies, y=S2_Spike_Ratio, color=Cycle_numbers)) +
  geom_quasirandom(alpha=1, aes(color=Cycle_numbers),size=3) +
  geom_smooth(data=subset(test.wide,as.numeric(ATCC_Virus_copies) > 1),method=lm,se=FALSE) +
...
test.wide %>%
  mutate(ATCC_Virus_copies = if_else(ATCC_Virus_copies == 0, 3.162278, as.numeric(ATCC_Virus_copies))) %>%
  ggplot(aes(x=ATCC_Virus_copies, y=S2_Spike_Ratio, color=Taqpaths)) +
  geom_quasirandom(alpha=1, aes(color=Taqpaths),size=3) +
  geom_smooth(data=subset(test.wide,as.numeric(ATCC_Virus_copies) > 1),method=lm,se=FALSE) +
  geom_vline(xintercept = 5.623413) +
...
test.wide %>%
  mutate(ATCC_Virus_copies = if_else(ATCC_Virus_copies == 0, 3.162278, as.numeric(ATCC_Virus_copies))) %>%
  ggplot(aes(x=ATCC_Virus_copies, y=S2_Spike_Ratio, color=Virus_Heat_Inactivation_time)) +
  geom_quasirandom(alpha=1, aes(color=Virus_Heat_Inactivation_time),size=3) +
  geom_smooth(data=subset(test.wide,as.numeric(ATCC_Virus_copies) > 1),method=lm,se=FALSE) +
  geom_vline(xintercept = 5.623413) +
  scale_color_manual(values=c('#FDE725FF','#2D708EFF','#440154FF')) +
  scale_x_log10(breaks = c(10^(-1:4)), labels = c(0,10^(0:4))) +
  scale_y_log10() +
  annotation_logticks() +
  theme(legend.position='none')
...
write.table(test.wide,"Saliva_dilution_various_conditions_wide_mat.txt",row.names=F,quote=F,sep="\t")
