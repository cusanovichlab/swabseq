...
counts = read.table("Gargle_Inactivation.data.txt")
counts = counts[,-2]
metadata = read.table("Gargle_Inactivation.metadata.txt",header=T,sep="\t")
...
test.wide = test.wide[,c(1,5:8,15,9,11,16:19)]
colnames(test.wide)[1] = "Index"
colnames(test.wide)[2] = "Index1"
colnames(test.wide)[3] = "Index2"

pdf("65vs95.pdf",height=8,width=8)
test.wide %>%
  mutate(ATCC_virus_copies = if_else(ATCC_virus_copies == 0, 3.162278, as.numeric(ATCC_virus_copies))) %>%
  ggplot(aes(x=ATCC_virus_copies, y=S2_Spike_Ratio, color=Plate_config)) +
  geom_quasirandom(alpha=1, aes(color=Plate_config),size=3) +
  geom_smooth(data=subset(test.wide,as.numeric(ATCC_virus_copies) > 0),method=lm,se=FALSE) +
  geom_vline(xintercept = 5.623413) +
  scale_color_manual(values=c('#440154FF','#2D708EFF','#55C667FF','#FDE725FF')) +
...
test.wide %>%
  mutate(ATCC_virus_copies = if_else(ATCC_virus_copies == 0, 3.162278, as.numeric(ATCC_virus_copies))) %>%
  ggplot(aes(x=ATCC_virus_copies, y=S2_Spike_Ratio, group=ATCC_virus_copies)) +
...
write.table(test.wide,"Gargle_Inactivation_wide_mat.txt",row.names=F,quote=F,sep="\t")
